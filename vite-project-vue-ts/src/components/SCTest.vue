...
import { checkBalance, awaitTxConfirmation, deploySmartContracts } from "../utils/Deployer.ts"
...
class MusicAlbum implements ISerializable<MusicAlbum> {
  ...
  public deserialize(data: Uint8Array, offset: number): IDeserializedResult<MusicAlbum> {
...
export default {
  ...
  methods: {
    ...
    async pollAsyncEvents(web3Client: Client, opId: string): Promise<IEventPollerResult> {
    ...
    async init() {
      ...
      const deployerPrivateKey = this.deployerPrivateKey;
      ...
      try {
        ...
        console.log(
          `Deployer Wallet Address: ${deployerAccount.address
        ...
        const { isError, eventPoller, events }: IEventPollerResult =
          await withTimeoutRejection<IEventPollerResult>(
            this.pollAsyncEvents(web3Client, deploymentOperationId),
            20000
          );
